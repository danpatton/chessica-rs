...
use crate::{Piece, Side, Move, RegularMove, PromotionMove, EnPassantCaptureMove, sq};
use crate::bitboard_magic::{find_bishop_magics, find_rook_magics, MagicBitBoardTable, MAGIC_INDEX_BITS};
...
impl MoveUndoInfo {
    fn new(white: &BoardSide, black: &BoardSide, half_move_clock: u8, ep_square: Option<Square>) -> Self {
        MoveUndoInfo {
        ...
        match piece {
            ...
            Piece::Rook => {
                ...
                if from.rank() == starting_rank {
                    match from.file() {
                        0 => { self.castling_rights &= !LONG_CASTLING; }
                        7 => { self.castling_rights &= !SHORT_CASTLING; }
                        _ => {}
                    };
                }
            },
...
impl Board {
    ...
    pub fn parse_fen(fen: &str) -> Result<Self, FenParseError> {
        lazy_static! {
            static ref RE: Regex = Regex::new(r"([RNBQKPrnbqkp1-8/]{15,}) ([wb]) (K?Q?k?q?|-) ([a-h]?[1-8]?|-) (\d+) (\d+)").unwrap();
        }
        if let Some(captures) = RE.captures(fen) {
            ...
            let side_to_move = if &captures[2] == "w" { Side::White } else { Side::Black };
    ...
    pub fn to_fen_string(&self) -> String {
        ...
        sb.append(if self.side_to_move == Side::White { " w " } else { " b " });
        ...
        sb.append(format!(" {} {}", self.half_move_clock, self.full_move_number));
    ...
    pub fn push(&mut self, move_: &Move) {
        let move_undo_info = MoveUndoInfo::new(
            &self.white_side, &self.black_side, self.half_move_clock, self.ep_square);
        self.ep_square = None;
        match move_ {
            Regular(m) => {
        ...
        self.side_to_move = if self.side_to_move == Side::White { Side::Black } else { Side::White };
    ...
    pub fn pop(&mut self) {
        if let Some((move_, move_undo_info)) = self.move_stack.pop() {
            let prev_move_side = if self.side_to_move == Side::White { Side::Black } else { Side::White };
            match move_ {
            ...
            self.white_side.set_castling_rights(move_undo_info.white_castling_rights());
            self.black_side.set_castling_rights(move_undo_info.black_castling_rights());
    ...
    pub fn legal_moves(&self) -> Vec<Move> {
        ...
        let check_evasion_mask = if in_check { checks.checking_pieces | checks.check_blocking_squares } else { BitBoard::full() };

        for own_bishop_or_queen in own_side.bishops | own_side.queens {
            let mut allowed_moves = self.bishop_moves(own_bishop_or_queen, all_pieces) & !own_pieces & check_evasion_mask;
            ...
            let piece = if own_side.bishops.is_occupied(own_bishop_or_queen) { Piece::Bishop } else { Piece::Queen };
        ...
        for own_rook_or_queen in own_side.rooks | own_side.queens {
            let mut allowed_moves = self.rook_moves(own_rook_or_queen, all_pieces) & !own_pieces & check_evasion_mask;
            if allowed_moves.any() {
            ...
            let piece = if own_side.rooks.is_occupied(own_rook_or_queen) { Piece::Rook } else { Piece::Queen };
            for square in allowed_moves {
                moves.push(Regular(RegularMove{
                    piece,
        ...
        for own_pawn in diagonally_pinned_pawns {
            let capture_mask = BitBoard::empty().set(own_pawn).pawn_captures(own_side.side) & enemy_pieces;
        ...
        for own_pawn in orthogonally_pinned_pawns {
            let push_mask = BitBoard::empty().set(own_pawn).pawn_pushes(own_side.side) & !all_pieces;
            ...
            let double_push_mask = push_mask.pawn_pushes(own_side.side) & double_push_rank & !all_pieces;
        ...
        let pawn_pushes = unpinned_pawns.pawn_pushes(own_side.side) & !all_pieces & check_evasion_mask;
        ...
        let pawn_double_push_rank = BitBoard::rank(if own_side.side == Side::White { 3 } else { 4 });
        let pawn_double_pushes = (unpinned_pawns.pawn_pushes(own_side.side) & !all_pieces).pawn_pushes(own_side.side) & pawn_double_push_rank & !all_pieces & check_evasion_mask;
        let pawn_double_pushees = pawn_double_pushes.pawn_pushes(enemy_side.side).pawn_pushes(enemy_side.side);
        ...
        let pawn_left_captures_excl_ep = unpinned_pawns.pawn_left_captures(own_side.side) & enemy_pieces & check_evasion_mask;
        let pawn_left_capturers = pawn_left_captures_excl_ep.pawn_left_captures(enemy_side.side);
        let pawn_right_captures_excl_ep = unpinned_pawns.pawn_right_captures(own_side.side) & enemy_pieces & check_evasion_mask;
        let pawn_right_capturers = pawn_right_captures_excl_ep.pawn_right_captures(enemy_side.side);

        let pawn_captures = pawn_left_capturers.zip(pawn_left_captures_excl_ep).chain(pawn_right_capturers.zip(pawn_right_captures_excl_ep));
    ...
    fn bishop_moves(&self, square: Square, all_pieces: BitBoard) -> BitBoard {
        lazy_static! {
            static ref BISHOP_MAGICS: Vec<MagicBitBoardTable> = find_bishop_magics(MAGIC_INDEX_BITS);
        }
    ...
    fn attacked_squares(&self) -> BitBoard {
        ...
        let all_pieces_except_own_king = (own_side.all_pieces() | enemy_side.all_pieces()) & !own_side.king;
    ...
    fn checks(&self, all_pieces: BitBoard) -> Checks {
        ...
        let checking_diag_sliders = (enemy_side.bishops | enemy_side.queens) &
            self.bishop_moves(own_king, all_pieces);
        let checking_orthog_sliders = (enemy_side.rooks | enemy_side.queens) &
            self.rook_moves(own_king, all_pieces);
